...
| app:session_idle_timeout    | APP__SESSION__IDLE__TIMEOUT                     | 0                        | Session Idle Timeout, in ms (default 0 - disabled). This timeout controls the launch time of a popup message due to user inactivity that will alert the user to a pending session logout when app:session_timeout is reached. When the app:session_idle_timeout popup is launched, the OpenCTI desktop work area will be blurred and a countdown to logout will begin within the popup.                                | 
